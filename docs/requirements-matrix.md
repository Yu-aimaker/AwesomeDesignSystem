...
| ------- | ------------------------------------------------------------------ | -------------------- | -------------------------------------------------------------------- | ------------------------------------------------- | --------------- |
| REQ-001 | Product definition (bible + atlas + libs + site + skills + QA)     | platform             | monorepo root, packages/*, apps/docs                                 | `pnpm qa:full`                                    | done            |
...
| REQ-004 | Principle → practice mapping                                       | content/react/motion | shared contracts + recipe rule IDs + test verification edges         | graph + compiler API gate                         | done            |
| REQ-005 | Framework-independent core                                         | tokens/core          | CSS vars, contracts, no React in core                                | package tests                                     | done            |
| REQ-006 | WCAG 2.2 AA by construction                                        | react/docs           | component tests, axe, a11y suite                                     | `pnpm qa:full`                                    | done            |
| REQ-007 | Global/JP typography + RTL                                         | docs/react           | JP mobile + RTL + contrast visual fixtures                           | 12-case visual + reflow tests                     | done            |
...
| REQ-010 | Freshness visibility                                               | content/scripts      | content observations + GitHub adapter health + queue + `/status`     | CLI lifecycle fixture + double-run + strict links | done            |
...
| REQ-014 | Component baseline set                                             | react                | 32 shared contracts + built ESM/types + live previews                | unit + registry crawl + package smoke             | done            |
...
| REQ-017 | Bidirectional evidence graph                                       | content              | refs/rules/artifacts/test edges + `/rules/[id]` trace UI             | negative graph fixtures + drift checks + E2E      | done            |
...
| REQ-020 | Unresolved Critical/High = 0 at completion                         | qa                   | architecture/spec/UI fixed-point reviews + dependency graph          | independent re-audit + `qa:full` + audit          | done            |
| REQ-021 | English/Japanese website localization                              | docs                 | full Brand/Canon/Reference/component chrome + explicit fallback      | i18n + E2E + visual + axe + metadata              | done            |
