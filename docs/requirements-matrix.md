...
| REQ-004 | Principle → practice mapping                                       | content/react/motion | shared component contracts + recipe rule IDs                         | graph + registry validation              | done            |
...
| REQ-010 | Freshness visibility                                               | content/scripts      | HTTP/GitHub observations + review queue + `/status`                  | freshness double-run + strict links      | done            |
...
| REQ-020 | Unresolved Critical/High = 0 at completion                         | qa                   | sanitizer + patched dependency graph                                 | `pnpm audit --audit-level low` + full QA | done            |
| REQ-021 | English/Japanese website localization                              | docs                 | localized dynamic detail chrome + explicit English fallback          | i18n + dynamic-route E2E + visual + axe  | done            |
