...
import { getContentRoot } from "../lib/path-resolver";

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const base = getSiteUrl().origin;
  const localizedEntries = (route: string) =>
    locales.map((locale) => ({
        ...
        languages: {
          ...Object.fromEntries(
            locales.map((language) => [
              language,
              base + localizePathname(route, language),
            ]),
          ),
  ...
  const [refs, rules, artifacts] = await Promise.all([
  ...
  return [
    ...
    ...motionRecipes.flatMap((recipe) =>
      localizedEntries("/motion/" + recipe.intent),
    ),
    ...refs.flatMap((reference) =>
      localizedEntries("/references/" + encodeURIComponent(reference.id)),
    ),
    ...rules.flatMap((rule) =>
      localizedEntries("/rules/" + encodeURIComponent(rule.id)),
    ),
    ...artifacts.flatMap((artifact) =>
      localizedEntries("/artifacts/" + encodeURIComponent(artifact.id)),
    ),
  ];
}
