...
describe("semantic tokens", () => {
  ...
  test("framework accent default is brand-neutral, not the AwesomeDS CMY magenta", () => {
    // The portable package must not ship AwesomeDS's CMY/magenta identity as its
    // default; the docs brand layer applies brand color by overriding the same
    // semantic role. The neutral placeholder is a blue (hue 255).
    for (const theme of [lightTheme, darkTheme, highContrastTheme]) {
      expect(theme["color-accent"]).toContain("255");
    }
    expect(lightTheme["color-accent"]).not.toContain(" 350)");
    expect(darkTheme["color-accent"]).not.toContain(" 350)");
  });
