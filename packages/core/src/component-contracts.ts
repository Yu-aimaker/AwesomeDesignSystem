export type ComponentCatalogItem = {
  ...
  testIds: string[];
  testEvidence: Array<{ file: string; caseName: string; level: "unit" | "browser" }>;
  publicApi: Array<{ name: string; type: string; required: boolean; description: string }>;
};
...
const details: Record<string, ContractDetail> = {
  "button": { anatomy: ["native button", "label", "loading indicator"], keyboard: "Enter and Space activate once; disabled and loading states suppress activation.", screenReader: "The native button name remains stable while aria-disabled and busy state are exposed.", contentRules: ["Begin labels with a specific verb.", "Keep loading text consistent with the action."], publicApi: [prop("children", "ReactNode", true, "Visible accessible label"), prop("loading", "boolean", false, "Prevents duplicate actions")], testIds: ["Button/loading-disabled", "Button/native-activation"] },
  "icon-button": { anatomy: ["native button", "decorative icon", "accessible label"], keyboard: "Enter and Space activate the control; focus uses the shared focus ring.", screenReader: "label supplies the required accessible name and the icon is presentation-only.", contentRules: ["Use a familiar single-purpose icon.", "Describe the action, not the icon shape."], publicApi: [prop("label", "string", true, "Accessible action name"), prop("children", "ReactNode", true, "Icon artwork")], testIds: ["IconButton/requires-label"] },
  "link": { anatomy: ["anchor", "link text", "optional external cue"], keyboard: "Tab focuses; Enter follows the href using native anchor behavior.", screenReader: "Announces as a link with meaningful destination text.", contentRules: ["Name the destination.", "Avoid bare URLs and vague read-more labels."], publicApi: [prop("href", "string", true, "Navigation destination"), prop("children", "ReactNode", true, "Destination label")], testIds: ["Link/renders-href"] },
  "badge": { anatomy: ["inline status container", "short label"], keyboard: "Not focusable unless nested in an interactive owner.", screenReader: "Text is read in document order; decorative badges must be hidden by the caller.", contentRules: ["Use one or two short words.", "Do not place critical meaning only in tone."], publicApi: [prop("tone", "BadgeTone", false, "Semantic visual tone"), prop("children", "ReactNode", true, "Status text")], testIds: ["Badge/tone-text"] },
  "input": { anatomy: ["label", "text input", "hint", "error message"], keyboard: "Uses native text editing, selection, and Tab order.", screenReader: "Label, hint, invalid state, and error are programmatically associated.", contentRules: ["Labels describe the requested value.", "Errors explain how to recover."], publicApi: [prop("id", "string", true, "Association identifier"), prop("label", "string", true, "Field name"), prop("error", "string", false, "Recovery message")], testIds: ["Input/label-error-association"] },
  "textarea": { anatomy: ["label", "multiline text area", "hint or error"], keyboard: "Native multiline editing; Enter inserts a newline rather than submitting implicitly.", screenReader: "Label and descriptive/error text are associated with the textarea.", contentRules: ["State expected content and limits.", "Do not use placeholder text as a label."], publicApi: [prop("id", "string", true, "Association identifier"), prop("label", "string", true, "Field name")], testIds: ["Textarea/multiline-label"] },
  "checkbox": { anatomy: ["native checkbox", "visible label", "optional description"], keyboard: "Space toggles the focused checkbox; Tab moves to the next control.", screenReader: "Announces checkbox name, checked/mixed state, and disabled state.", contentRules: ["Write the label as a selectable statement.", "Use a group legend for related choices."], publicApi: [prop("id", "string", true, "Association identifier"), prop("label", "string", true, "Choice label")], testIds: ["Checkbox/native-toggle"] },
  "switch": { anatomy: ["switch control", "track and thumb", "persistent label"], keyboard: "Space toggles immediately; Enter may toggle through the native button implementation.", screenReader: "role=switch exposes its name and aria-checked state.", contentRules: ["Name the setting, not On or Off.", "Use only for immediate changes."], publicApi: [prop("id", "string", true, "Association identifier"), prop("label", "string", true, "Setting name")], testIds: ["Switch/role-state"] },
  "radio-group": { anatomy: ["fieldset", "legend", "radio options"], keyboard: "Arrow keys move selection within the group; Tab enters and leaves the group.", screenReader: "Legend names the radiogroup and each option exposes checked state.", contentRules: ["Options must be mutually exclusive.", "Use concise parallel labels."], publicApi: [prop("legend", "string", true, "Group question"), prop("options", "RadioOption[]", true, "Available answers")], testIds: ["RadioGroup/single-selection"] },
  "select": { anatomy: ["label", "native select", "options", "error message"], keyboard: "Uses platform-native select keyboard and type-ahead behavior.", screenReader: "Label, selected option, invalid state, and error are announced.", contentRules: ["Use recognizable option labels.", "Avoid a disabled placeholder as the only instruction."], publicApi: [prop("id", "string", true, "Association identifier"), prop("label", "string", true, "Field name")], testIds: ["Select/native-options"] },
  "dialog": { anatomy: ["modal overlay", "labelled dialog", "title", "content", "close action"], keyboard: "Focus moves inside, Tab is contained, Escape closes, and focus returns to the trigger.", screenReader: "role=dialog and the title identify a modal context.", contentRules: ["Use a task-specific title.", "Keep the primary action visible and unambiguous."], publicApi: [prop("open", "boolean", true, "Controlled visibility"), prop("onClose", "() => void", true, "Dismiss callback"), prop("title", "string", true, "Accessible title")], testIds: ["Dialog/escape-focus-restore"] },
  "alert-dialog": { anatomy: ["modal overlay", "alertdialog", "consequence", "confirm action", "cancel action", "error alert"], keyboard: "Focus is contained; Escape cancels unless confirmation is pending; actions lock during pending work.", screenReader: "role=alertdialog announces the title, consequence, pending state, and rejected confirmation error.", contentRules: ["Name the irreversible consequence.", "Use explicit confirm and safe cancel labels."], publicApi: [prop("onConfirm", "() => void | Promise<void>", true, "Destructive operation"), prop("confirming", "boolean", false, "Externally controlled pending state")], testIds: ["AlertDialog/confirm-cancel", "AlertDialog/pending-rejection", "AlertDialog/escape-focus-restore"] },
  ...
  "skeleton": { anatomy: ["geometry-matched placeholder blocks"], keyboard: "Never focusable and never blocks keyboard navigation to already available content.", screenReader: "Decorative geometry is hidden; the owning region supplies one loading announcement.", contentRules: ["Match the expected layout.", "Do not use skeletons for sub-second updates."], publicApi: [prop("height", "CSSProperties['height']", false, "Expected block height")], testIds: ["Skeleton/hidden-placeholder"] },
  "spinner": { anatomy: ["status container", "animated indicator", "accessible label"], keyboard: "Not focusable; completion returns focus only when the workflow requires it.", screenReader: "A concise loading label is exposed without announcing every animation frame.", contentRules: ["Name what is loading.", "Avoid indefinite blocking without recovery."], publicApi: [prop("label", "string", true, "Loading status")], testIds: ["Spinner/status-label"] },
...
export const componentCatalog: ComponentCatalogItem[] = componentCatalogSource.map((item) => {
  ...
  return {
    ...
    testEvidence: [{ file: "tests/e2e/component-catalog.spec.ts", caseName: "all component contracts have a working localized detail and live preview", level: "browser" }],
  };
});
